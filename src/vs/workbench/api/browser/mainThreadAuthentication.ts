...
import { mcpEnterpriseManagedAuthClientIdSection, mcpEnterpriseManagedAuthClientSecretSection } from '../../contrib/mcp/common/mcpConfiguration.js';
	...
	constructor(
		...
		this._register(authenticationService.registerAuthenticationProviderHostDelegate({
			...
			createXaa: async (issuer) => {
				...
				// fallback — so an explicit setting is the most reliable source.
				const configuredClientId = (this.configurationService.getValue<string | undefined>(mcpEnterpriseManagedAuthClientIdSection) || '').trim() || undefined;
				const configuredClientSecret = (this.configurationService.getValue<string | undefined>(mcpEnterpriseManagedAuthClientSecretSection) || '').trim() || undefined;
