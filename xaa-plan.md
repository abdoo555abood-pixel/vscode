...
## Status (verified end-to-end against xaa.dev)

Tested against `https://idp.xaa.dev` / `https://auth.resource.xaa.dev` / `https://mcp.xaa.dev/mcp` on 2026-05-20.

- ✅ Step 1 — User signs into IdP via Auth Code + PKCE
- ✅ Step 2 — IdP token exchange → ID-JAG (`subject_token_type=urn:ietf:params:oauth:token-type:id_token`, `requested_token_type=urn:ietf:params:oauth:token-type:id-jag`)
- ✅ Step 3 — Resource AS redeems ID-JAG → access token (`grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer`)
- ✅ MCP server connects with `Authorization: Bearer …` and returns 200 OK on the streaming endpoint
- ✅ Subsequent connects are silent (no browser, no prompt)

### Required configuration (v1)

Three settings + two mcp.json fields. The XAA spec requires _two_ OAuth client registrations (one at the IdP, one at each resource AS), so both need to be surfaced.

**`settings.json`** (application scope):
```json
{
  "mcp.enterpriseManagedAuth.issuer":       "https://idp.example.com",
  "mcp.enterpriseManagedAuth.clientId":     "<client_id at the IdP>",
  "mcp.enterpriseManagedAuth.clientSecret": "<client_secret at the IdP>"
}
```
  ...
  "servers": {
    "my-mcp": {
      ...
      "oauth": {
        ...
        "clientId":     "<client_id at the resource AS>",
        "clientSecret": "<client_secret at the resource AS>"
      }
    }
  }
}
...
When the IdP and the resource AS happen to use the same client_id (i.e. xaa.dev's `{idp_client_id}-at-{resource}` pattern is _not_ in play), both `oauth.clientId/clientSecret` may be omitted and the IdP creds will be reused.

### Follow-ups

- **Reduce config surface**: the per-resource client creds are unergonomic. Ideal future state is one of:
  - resource AS implements DCR so we register dynamically and drop both `oauth.clientId/clientSecret`, OR
  - we lift the IdP-managed client and have the resource AS trust it directly (no separate registration).
- **Policy / MDM-managed issuer**: the three IdP settings should be lockable per-machine.
- **Scope plumbing**: at the ID-JAG step we currently omit `scope` (RFC 8693 §2.1 allows this; xaa.dev's AS-default behavior worked). MCP servers that need per-resource scopes will surface them via the WWW-Authenticate / resource metadata `scopes_supported` chain — that's already plumbed for step 3 but is _not_ forwarded to step 2 yet.
